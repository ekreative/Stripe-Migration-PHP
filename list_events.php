...
if (file_exists('event_checked_customers.json')) {
    echo "loaded event checked customers\n";
    $dataR = file_get_contents('event_checked_customers.json');
    $eventCheckedCustomers = json_decode($dataR, true);
}
...
do {
    ...
    $criteria = ['limit' => 100, 'type' => 'customer.subscription.updated'];
    // Pagination
    ...
    $events = \Stripe\Event::all($criteria);
    $progress->advance();

    $customers = [];
    foreach ($events['data'] as $event) {
        if (isset($events['previous_attributes']['cancel_at_period_end']) && $events['previous_attributes']['cancel_at_period_end'] === false) {
            $customers[] = [
